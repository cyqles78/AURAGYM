...
export const ActiveWorkoutScreen: React.FC<ActiveWorkoutScreenProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#000000] flex flex-col animate-in slide-in-from-bottom">
        ...
        {/* Exercises List */}
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-4 space-y-8 pb-32 relative">
            
            {/* Pause Overlay */}
            {isPaused && (
                <div className="absolute inset-0 z-20 bg-black/60 backdrop-blur-sm flex items-center justify-center">
                    <div className="bg-[#1C1C1E] border border-yellow-500/30 p-8 rounded-3xl text-center shadow-2xl">
                         ...
                         <button 
                            onClick={togglePause}
                            className="w-full py-3 bg-white text-black font-bold rounded-xl flex items-center justify-center gap-2 hover:bg-gray-200"
                         >
            ...
            {(session.exercises || []).map((exercise, exIdx) => (
                <div key={exercise.id + exIdx} className="bg-[#1C1C1E] border border-[#2C2C2E] rounded-2xl p-4 space-y-4">
