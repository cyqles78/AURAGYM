...
import { WorkoutSession, ExercisePerformanceEntry, Exercise } from '../types';
import { useActiveWorkout } from '../hooks/useActiveWorkout';
import { RestTimer } from '../components/RestTimer';
import { PlateCalculator } from '../components/PlateCalculator';
import { Minus, Plus, CheckCircle2, MoreHorizontal, Clock, Play, Pause, Trash2, SkipForward, Calculator, History, Flame, StickyNote, Copy, ChevronDown, Link } from 'lucide-react';
...
export const ActiveWorkoutScreen: React.FC<ActiveWorkoutScreenProps> = ({ 
  ...
  const exerciseGroups = useMemo(() => {
    const groups: { isSuperset: boolean; id?: string; exercises: { data: Exercise; index: number }[] }[] = [];
    
    if (!session.exercises) return [];

    let currentGroup: { isSuperset: boolean; id?: string; exercises: { data: Exercise; index: number }[] } | null = null;

    session.exercises.forEach((ex, idx) => {
        if (ex.supersetId) {
            if (currentGroup && currentGroup.isSuperset && currentGroup.id === ex.supersetId) {
                // Continue existing superset
                currentGroup.exercises.push({ data: ex, index: idx });
            } else {
                // Push previous group if exists
                if (currentGroup) groups.push(currentGroup);
                // Start new superset
                currentGroup = { isSuperset: true, id: ex.supersetId, exercises: [{ data: ex, index: idx }] };
            }
        } else {
            // Push previous group if exists
            if (currentGroup) groups.push(currentGroup);
            // Single exercise group
            currentGroup = { isSuperset: false, exercises: [{ data: ex, index: idx }] };
        }
    });
    if (currentGroup) groups.push(currentGroup);
  ...
  useEffect(() => {
    if (activeSetFocus) {
        const refKey = `${activeSetFocus.exerciseIndex}-${activeSetFocus.setIndex}`;
        const el = exerciseRefs.current[refKey];
        if (el) {
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
            ...
            <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-4 space-y-8 pb-32">
                ...
                {exerciseGroups.map((group, groupIdx) => {
                    ...
                    if (group.isSuperset) {
                        return (
                            <div key={`group-${groupIdx}`} className="relative pl-4">
                                {/* Visual Connector */}
                                <div className="absolute left-0 top-2 bottom-2 w-1 bg-accentBlue rounded-full" />
                                <div className="absolute -left-3 top-1/2 -translate-y-1/2 bg-[#1C1C1E] p-1.5 rounded-full border border-accentBlue text-accentBlue z-10 shadow-lg">
                                    <Link size={14} />
                                </div>
                                
                                <div className="space-y-4">
                                    {group.exercises.map((item, itemIdx) => {
                                        const alphabetLabel = String.fromCharCode(65 + itemIdx); // A, B, C...
                                        return (
                                            <div key={item.data.id + item.index} className="relative">
                                                {/* Alphabet Badge */}
                                                <div className="absolute -left-9 top-6 bg-accentBlue text-black font-bold text-[10px] h-5 w-5 rounded-full flex items-center justify-center z-20 border border-[#1C1C1E]">
                                                    {groupIdx + 1}{alphabetLabel}
                                                </div>
                                                <ExerciseCard 
                                                    exercise={item.data} 
                                                    exIdx={item.index}
                                                    lastSession={getLastSession(item.data.name)}
                                                    isPaused={isPaused}
                                                    isSuperset={true}
                                                    {...{ toggleSetComplete, updateSetField, addSet, deleteSet, skipExercise, addWarmupSets, copySetsFromHistory, updateExerciseNote, openCalculator, exerciseRefs }}
                                                />
                                            </div>
                                        );
                                    })}
...
// Extracted Card Component for cleaner rendering logic
const ExerciseCard: React.FC<any> = ({ 
    exercise, exIdx, lastSession, isPaused, isSuperset,
    toggleSetComplete, updateSetField, addSet, deleteSet, skipExercise, addWarmupSets, copySetsFromHistory, updateExerciseNote, openCalculator, exerciseRefs 
}) => {
    const lastBestSet = lastSession?.sets.reduce((prev: any, curr: any) => (curr.weight || 0) > (prev.weight || 0) ? curr : prev, lastSession.sets[0]);

    return (
        <div className={`bg-[#1C1C1E] border ${isSuperset ? 'border-accentBlue/20' : 'border-[#2C2C2E]'} rounded-2xl p-4 space-y-4 shadow-lg`}>
            ...
            <div className="space-y-2">
                {(exercise.sets || []).map((set: any, setIdx: number) => {
                    ...
                    return (
                        <div 
                            key={set.id} 
                            // Assign Ref for auto-scroll logic
                            ref={el => { if (exerciseRefs && exerciseRefs.current) exerciseRefs.current[`${exIdx}-${setIdx}`] = el }}
                        ...
                        >
                            <span className={`font-bold text-center rounded-md py-2.5 text-xs ${isWarmup ? 'text-orange-500 bg-orange-500/10' : 'text-[#8E8E93] bg-[#2C2C2E]'}`}>
                                {isWarmup ? 'W' : setIdx + 1 - (exercise.sets.filter((s: any) => s.isWarmup).length)}
                            </span>
