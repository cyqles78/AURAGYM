        ...
        <div className="pb-24 pt-4 space-y-6">
            <div className="flex justify-between items-center px-1">
                ...
                <button
                    ...
                    className="flex items-center gap-1 bg-white text-black px-3 py-2 rounded-full text-xs font-bold active:scale-95 transition shadow-sm"
                >
            ...
            <div className="space-y-3">
                <h2 className="text-base font-bold text-white flex items-center gap-2 px-1"><BarChart2 size={16} /> Training Insights</h2>

                {/* Horizontal Scroll Container for Stats */}
                <div className="flex gap-3 overflow-x-auto pb-3 px-1 no-scrollbar mobile-scroll snap-x snap-mandatory">

                    {/* 1. Volume Chart */}
                    <GlassCard className="min-w-[280px] w-[82vw] snap-center flex-shrink-0">
                        <div className="flex justify-between items-center mb-3">
                            <h3 className="text-sm font-bold text-white">Weekly Volume</h3>
                            <span className="text-[10px] text-secondary font-medium">Last 7 Days</span>
                        </div>
                        <div className="h-28 w-full">
                            {weeklyVolumeData.every(d => d.volume === 0) ? (
                                <div className="h-full flex items-center justify-center text-[10px] text-secondary italic border border-dashed border-white/10 rounded-lg">
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={weeklyVolumeData}>
                                        ...
                                        <Tooltip
                                            ...
                                            contentStyle={{ backgroundColor: '#1C1C1E', borderColor: '#2C2C2E', borderRadius: '8px', fontSize: '11px' }}
                                            itemStyle={{ color: '#fff' }}
                                        />
                                        <Area type="monotone" dataKey="volume" stroke="#FFFFFF" strokeWidth={2} fill="url(#colorVol)" />
                                        <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: '#8E8E93', fontSize: 9 }} dy={8} />
                                    </AreaChart>
                                </ResponsiveContainer>
                            )}
                        </div>
                    ...
                    {/* 2. Muscle Frequency */}
                    <GlassCard className="min-w-[240px] w-[68vw] snap-center flex-shrink-0">
                        <h3 className="text-sm font-bold text-white mb-3">Top Muscles</h3>
                        <div className="space-y-2.5">
                            {muscleFreqData.length > 0 ? muscleFreqData.map((m, i) => (
                                ...
                                <div className="h-20 flex items-center justify-center text-[10px] text-secondary italic border border-dashed border-white/10 rounded-lg">
                                    No recent data
                                </div>
                            )}
                        </div>
                    ...
                    {/* 3. Recent PRs */}
                    <GlassCard className="min-w-[240px] w-[68vw] snap-center flex-shrink-0">
                        <h3 className="text-sm font-bold text-white mb-3">Recent Records</h3>
                        <div className="space-y-2.5">
                            {prProgressData.length > 0 ? prProgressData.map((pr, i) => (
                                <div key={pr.name} className="flex justify-between items-center border-b border-white/5 last:border-0 pb-2 last:pb-0">
                                    <div>
                                        <p className="text-xs font-bold text-white truncate max-w-[110px]">{pr.name}</p>
                                        <p className="text-[9px] text-secondary">Est. 1RM</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-sm font-bold text-white">{pr.latest} kg</p>
                                        {pr.improvement > 0 && (
                                            <span className="text-[9px] text-accentGreen flex items-center justify-end">
            ...
            <div className="space-y-3">
                <div className="flex justify-between items-center px-1">
                    <h2 className="text-base font-bold text-white flex items-center gap-2"><Calendar size={16} /> My Programs</h2>
                    <button
                        onClick={() => setViewMode('PROGRAM_BUILDER')}
                        className="text-xs font-bold text-black bg-white px-3 py-2 rounded-full flex items-center gap-1 active:scale-95 transition"
                    >
                        <Sparkles size={11} fill="black" /> New
                    </button>
                </div>

                {programs.length === 0 ? (
                    <GlassCard className="text-center py-6" onClick={() => setViewMode('PROGRAM_BUILDER')}>
                        <Layers className="mx-auto text-secondary mb-2 opacity-50" size={28} />
                        <p className="text-white font-bold mb-1 text-sm">No Active Programs</p>
                        <p className="text-[10px] text-secondary px-4">Use AI to build a multi-week schedule tailored to your goals.</p>
                    </GlassCard>
                ) : (
                    <div className="grid grid-cols-1 gap-3">
                        {programs.map(prog => (
                            <GlassCard key={prog.id} onClick={() => { setSelectedProgram(prog); setSelectedWeekIndex(0); setViewMode('PROGRAM_DETAIL'); }}>
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-secondary mt-0.5">{prog.goal} • {prog.durationWeeks} Weeks</p>
                                    </div>
                                ...
                                <div className="mt-3 flex gap-2">
                                    ...
                                    {(prog.weeks?.[0]?.days || []).slice(0, 3).map((d, i) => (
                                        <div key={i} className="h-1 flex-1 bg-surfaceHighlight rounded-full overflow-hidden">
            ...
            <div className="space-y-3">
                <div className="flex justify-between items-center px-1 border-t border-white/5 pt-5">
                    <h2 className="text-base font-bold text-white flex items-center gap-2"><Dumbbell size={16} /> Quick Workouts</h2>
                    <div className="flex gap-2">
                        <button
                            onClick={() => setViewMode('BUILDER')}
                            className="flex items-center gap-1 bg-surfaceHighlight border border-white/10 px-3 py-2 rounded-full text-xs font-bold text-white active:scale-95 transition"
                        >
                            <Hammer size={11} /> Builder
                        </button>
                        <button onClick={() => setViewMode('GENERATOR')} className="h-9 w-9 rounded-full bg-surfaceHighlight border border-border flex items-center justify-center text-white active:scale-95 transition">
                            <Plus size={16} />
                        </button>
                    </div>
                </div>
                    ...
                    <GlassCard key={plan.id} className="group relative" onClick={() => handleStartWorkout(plan)}>
                        <div className="flex justify-between items-start mb-3">
                            <div>
                                <div className="flex space-x-2 mb-1.5">
                                    <span className="px-2 py-0.5 rounded text-[9px] font-bold bg-surfaceHighlight text-secondary uppercase tracking-wide">
                                        {plan.difficulty}
                                    </span>
                                    {plan.tags.includes('Custom') && (
                                        <span className="px-2 py-0.5 rounded text-[9px] font-bold bg-white text-black uppercase tracking-wide">
                                            Custom
                                        </span>
                                    )}
                                </div>
                                <h3 className="text-base font-bold text-white">{plan.title}</h3>
                                <div className="flex items-center text-[10px] text-secondary mt-0.5">
                                    <Clock size={10} className="mr-1" /> {plan.duration}
                                </div>
                            </div>
                            <div className="h-9 w-9 rounded-full bg-surfaceHighlight flex items-center justify-center group-active:bg-white group-active:text-black transition-colors">
                        ...
                        <div className="space-y-1">
                            ...
                            {plan.exercises.length > 3 && (
                                <div className="text-[10px] text-slate-600 pl-3">+{plan.exercises.length - 3} more</div>
                            )}
                        </div>
                    </GlassCard>
                ))}
